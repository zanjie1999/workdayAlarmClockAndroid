...
此时默认会显示时钟模式，可以从应用列表找到工作咩闹钟打开控制台，  
除了长按菜单和应用按钮，还可以通过按home键打开应用列表，按返回键可以回到之前的App
...
也可以长按方向键确认键或回车键打开；设置打开后可用方向键移动、确认键或回车键选择  
...
麦克风提供 AAC-LC fMP4 音频流，可以脱离摄像头单独播放。没有麦克风权限时该接口返回空内容：  
```
http://设备IP:8880/aac
http://设备IP:8880/abc/aac
```
...
长按日期可以锁屏  
当全屏时，上下左右滑动单击将控制播放
全屏收起状态下长按方向键确认键或回车键，可以恢复到收起前的控制界面  
...
<img width="1280" height="800" alt="image" src="https://github.com/user-attachments/assets/8de92b6d-7a15-4380-a721-4c5a14292a18" />  
长按中间的空白区域，可以进入设置，调节各个组件的位置和设置壁纸  
也可以长按方向键确认键或回车键打开设置；设置打开后可用方向键移动、确认键或回车键选择  
...
如果是常驻通知，只会在出现时通知   
通知转发设置中可以开启“亮屏时不推送”，开启后仅在收到通知时屏幕未亮起的情况下转发  
也可以在应用黑名单中勾选不需要转发通知的应用，列表同时会显示最近50个应用用于加入黑名单  

<img width="821" height="329" alt="image" src="https://github.com/user-attachments/assets/c7f8f4c2-2bda-43b0-8b52-5467d1e6b35a" />
...
合并多条通知时，会把 URL 最后一个 `=` 后面的内容作为单条通知模板，为每条通知分别替换标签，再使用换行连接，例如上面的模板会合并为：
```
https://abc.com/api?message=应用A包名是pkg.a发送了标题为标题A内容为内容A的通知%0A应用B包名是pkg.b发送了标题为标题B内容为内容B的通知
```
如果 URL 中没有 `=`，则继续将默认的“标题：内容”格式直接拼在 URL 末尾

## 待办事项 小组件
填写待办接口URL后，可以在大屏时钟模式使用待办事项组件  
支持两种接口返回格式：  
1. 纯文本
2. JsonArray
```
